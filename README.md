...
- **"RepairProtein"** There is currently not an option to skip protein repair.
    ...
    - **"preserve_resolved"**: If true, Modeller only optimizes the residues that are absent from the *input_pdb*; every residue with experimental coordinates is held fixed. Prevents resolved but flexible regions, such as receptor N-termini, from being remodelled away from the conformation they were solved in. Residues that are present but missing sidechain atoms are still optimized. Turn this on when the *input_pdb* resolves a flexible region whose conformation matters, e.g. a receptor N-terminus that contacts the ligand. Default is false.
    - **"secondary_template_gaps_only"**: If true, the *secondary_template* is superposed onto the input structure and trimmed to the residues that fall in gaps, so it only ever fills holes. Without this, Modeller builds restraints from both templates wherever they overlap and a predicted model can override the experimental coordinates. Only has an effect when a *secondary_template* is given. Default is false.
