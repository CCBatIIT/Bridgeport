...
class Simulation_Preparer(object):
    ...
    def __init__ (self, crystal_pdb_fn, ligand_resname):
        ...
        receptor_path, ligand_path = seperate_crys_using_MDA(crystal_pdb_fn, ligand_resname)
    ...
    def generate_topologies(self, save_as_jsons=True):
        ...
        self.comp_top = insert_molecule_and_remove_clashes(comp_top, ligand)
        ...
        solvent_phase_path = just_the_solvent(ligand_solvated_path, ligand_resname)
        solv_top = Topology.from_pdb(solvent_phase_path) #ligand cannot be here
        self.solv_top = insert_molecule_and_remove_clashes(solv_top, ligand) #and so is added here
        if save_as_jsons:
    ...
    def openmm_writeout(self):
        make_simulation_and_writeout(self.comp_interchange, 'complex')
        make_simulation_and_writeout(self.solv_interchange, 'solvent')